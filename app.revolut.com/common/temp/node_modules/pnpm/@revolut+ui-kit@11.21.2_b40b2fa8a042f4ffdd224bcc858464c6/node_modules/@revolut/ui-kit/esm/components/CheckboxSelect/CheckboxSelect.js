...
import { chain, isFunction, noop } from "../../utils";
...
import { useCheckboxSelect } from "./useCheckboxSelect";
import { CheckboxSelectLoadingState, CheckboxSelectType } from "./types";
export function CheckboxSelect(_ref) {
  var children = _ref.children,
      ...
      _ref$type = _ref.type,
      type = _ref$type === void 0 ? CheckboxSelectType.INSTANT : _ref$type,
      _ref$loadingState = _ref.loadingState,
      loadingState = _ref$loadingState === void 0 ? CheckboxSelectLoadingState.READY : _ref$loadingState,
      labelList = _ref.labelList,
      labelSelectAllProp = _ref.labelSelectAll,
      ...
      rest = _objectWithoutPropertiesLoose(_ref, ["children", "defaultValue", "disabled", "onChange", "value", "options", "open", "searchable", "onSearch", "onRetry", "retryAble", "onExited", "onRequestClose", "estimateSize", "type", "loadingState", "labelList", "labelSelectAll", "labelSearch", "labelApply", "labelClear", "labelRetry", "labelFailedState", "labelEmptyState", "labelNoResults"]);
  ...
  return /*#__PURE__*/React.createElement(AccessibleId, null, /*#__PURE__*/React.createElement(SelectBase, _extends({
    ...
    maxHeight: rem(getSearchBaseHeight(searchable, type === CheckboxSelectType.CONFIRM))
    ...
    onAction: function onAction() {
      var _checkRef$current;

      return (_checkRef$current = checkRef.current) == null ? void 0 : _checkRef$current.click();
    },
    ...
    indeterminate: select.value.length > 0,
    disabled: isDisabled || search.result.length === 0,
    onChange: function onChange() {
      // Manipulating large data with Set is much faster
      var set = new Set(select.value);
      var filtered = search.result.map(function (item) {
        return item.value;
      });

      if (filtered.every(function (item) {
        return set.has(item);
      })) {
    ...
    ready: search.result.length === 0 ? /*#__PURE__*/React.createElement(SelectBase.EmptyState, {
      ...
      return /*#__PURE__*/React.createElement(SelectBase.Option, {
        ...
        option: option,
        virtual: virtual,
        selected: select.value.includes(option.value),
        onFocus: setActive
      }, /*#__PURE__*/React.createElement(Checkbox, select.getProps({
        value: option.value,
        tabIndex: -1,
        hidden: true
      }), isFunction(children) ? children(option) : option.label));
    })
  }[loadingState], type === CheckboxSelectType.CONFIRM && /*#__PURE__*/React.createElement(SelectBase.Footer, null, /*#__PURE__*/React.createElement(Button, {
    size: "sm",
    width: "100%",
    variant: "secondary",
    ...
    disabled: isDisabled
  }, select.value.length > 0 ? chain(labelApply, select.value.length) : labelApply))));
}