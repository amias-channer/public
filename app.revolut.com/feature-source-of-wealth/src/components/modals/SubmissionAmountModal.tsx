...
type SubmissionAmountModalProps = {
  isOpen?: boolean
  onExit: VoidFunction
  onAdd: VoidFunction
}

export const SubmissionAmountModal: FC<SubmissionAmountModalProps> = ({
  isOpen = false,
  onExit,
  onAdd,
}) => {
  ...
  return (
    <Popup isOpen={isOpen} onExit={onExit} variant="colorful">
      <Header variant="form">
        ...
        <Header.Title>{t('SubmissionAmountModal.title')}</Header.Title>
        <Header.Description>{t('SubmissionAmountModal.description')}</Header.Description>
      </Header>
