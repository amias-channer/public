...
import { SubType, Values } from "../../lib/helpers/type"
...
export type IconVariant = SubType<Config, keyof typeof ICON_VARIANTS>
...
const VerifiedIcon = ({ className, size, variant = "VERIFIED" }: Props) => (
  <StyledSvg
    $size={size}
    $variant={variant}
    className={className}
...
const StyledSvg = styled.svg<{
  ...
  ${props =>
    variant({
      prop: "$variant",
      variants: {
        [ICON_VARIANTS.MODERATOR]: {
          ".VerifiedIcon--background": {
            fill: props.theme.colors.seaGrass,
          },
        },
        [ICON_VARIANTS.VERIFIED]: {
          ".VerifiedIcon--background": {
            fill: props.theme.colors.seaBlue,
          },
        },
      },
    })}
