...
export default function showBetaVersionSalutation({app, i18n, onContinue}: Props) {
    app.openModal({
        size: ModalSizes.MEDIUM,
        title: localize(i18n, 'trader.productTour.betaVersionSalutation.title'),
        content: (
            <>
                <img className={salutationModalImg} src={productBetaImgUrl} alt="" />
                <InnerHtml className={salutationModalContent}>
                    {localize(i18n, 'trader.productTour.betaVersionSalutation.description')}
                </InnerHtml>
            </>
        ),
        footer: (
