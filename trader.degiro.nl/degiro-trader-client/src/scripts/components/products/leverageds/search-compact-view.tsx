...
interface ProductsFrameProps {
    index: number;
    onData: LeveragedsSearchProps['onData'];
    searchParams: LeveragedsSearchProps['searchParams'];
}
...
const ProductsFrame = React.memo<ProductsFrameProps>(({index, searchParams, onData}) => {
    const {products, isLoading, error} = useCompactViewSearchParams(
        (config, currentClient, searchParams) => getLeveragedProducts(config, currentClient, searchParams),
        index,
        searchParams
    );
...
const LeveragedsSearchCompactView = React.memo<LeveragedsSearchProps>(({searchParams, onData}) => {
...
LeveragedsSearchCompactView.displayName = 'LeveragedsSearchCompactView';

export default LeveragedsSearchCompactView;
